...
const RelatedSearchInfo: React.FC<RelatedSearchInfoProps> = ({ data, setSectionVisible, setSearchValue }) => {
  ...
  return (
    <div className="flex flex-col gap-10 bg-gray-400 bg-opacity-50 bg-transparent text-center w-800 border-2 rounded-30 p-10">
      {data.map((data, idx) => (
        <div
          key={idx}
          className="hover:bg-white hover:text-gray-400 bg-opacity-50 text-20 text-white rounded-30 cursor-pointer"
